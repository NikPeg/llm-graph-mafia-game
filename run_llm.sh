...
# --- НАДЕЖНАЯ ОЧИСТКА ПЕРЕД ЗАПУСКОМ ---
# Используем pkill, который более надежен для поиска и остановки процессов
# Сначала проверяем, есть ли что останавливать, с помощью pgrep
# `&>/dev/null` скрывает любой вывод, нас интересует только код возврата
echo "Поиск и остановка любых предыдущих экземпляров vLLM..."
if pgrep -f "vllm serve" &>/dev/null; then
    echo "Найдены запущенные серверы vLLM. Отправляю команду на завершение..."
    # Используем pkill для отправки сигнала завершения
    pkill -f "vllm serve"
    echo "Ожидание завершения процессов (5 секунд)..."
    sleep 5
    # Финальная проверка и принудительная остановка, если что-то осталось
    if pgrep -f "vllm serve" &>/dev/null; then
        echo "Некоторые процессы не завершились. Принудительная остановка (pkill -9)..."
